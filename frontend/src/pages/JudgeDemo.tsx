...
export default function JudgeDemo() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans transition-colors duration-150 p-4 sm:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Link
                to="/"
                className="inline-flex items-center gap-1.5 text-slate-300 hover:text-white px-3 py-1 bg-slate-800 rounded-xl border border-slate-700 text-xs font-semibold"
              >
                <ArrowLeft size={14} /> Home
              </Link>
              <span className="bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          <div className={`p-6 rounded-3xl border-2 transition-all ${
            ...
            <div className="flex justify-between items-start mb-3">
              <span className="px-2.5 py-1 rounded-full bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 text-xs font-black uppercase tracking-wider">
                Scenario A
              </span>
              <span className="text-xs text-slate-400 font-bold">Rajesh Kumar</span>
            </div>
            <h3 className="text-lg font-black text-white mb-1">Normal Interaction Rhythm</h3>
            ...
            <button
              ...
              disabled={executing}
              className="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs flex items-center justify-center gap-2 cursor-pointer active:scale-98 transition-all"
            >
          ...
          <div className={`p-6 rounded-3xl border-2 transition-all ${
            selectedScenario === 'B'
              ? 'bg-slate-800 border-amber-500 ring-2 ring-amber-500/50'
              : 'bg-slate-950 border-slate-800 hover:border-slate-700'
          }`}>
            <div className="flex justify-between items-start mb-3">
              <span className="px-2.5 py-1 rounded-full bg-amber-950/80 border border-amber-500/40 text-amber-300 text-xs font-black uppercase tracking-wider">
                Scenario B
              </span>
              <span className="text-xs text-slate-400 font-bold">Sunita Devi</span>
            </div>
            <h3 className="text-lg font-black text-white mb-1">Intentional Degraded Interaction</h3>
            <p className="text-xs text-slate-400 leading-relaxed mb-4">
            ...
            <button
              ...
              disabled={executing}
              className="w-full py-3 px-4 rounded-xl bg-amber-600 hover:bg-amber-500 text-slate-950 font-black text-xs flex items-center justify-center gap-2 cursor-pointer active:scale-98 transition-all"
            >
              <Play size={14} fill="currentColor" />
        ...
        {stepIndex > 0 && (
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-950 border border-slate-800 mb-8 shadow-xl animate-in fade-in">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800">
              ...
              <button
                onClick={handleReset}
                className="px-3 py-1.5 rounded-lg border border-slate-700 hover:bg-slate-800 text-xs text-slate-400 flex items-center gap-1 font-bold"
              >
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-2xl border text-xs transition-all ${
                      isCurrent
                        ? 'border-blue-500 bg-blue-950/40 text-white font-extrabold ring-1 ring-blue-500'
                        : isPast
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black ${
                        isPast ? 'bg-emerald-500 text-slate-950' : isCurrent ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400'
                      }`}>
                        {isPast ? '✓' : idx + 1}
                      </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {features ? (
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-extrabold text-white">{Math.round(features.accuracy * 100)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Mean Response:</span>
                      <span className="font-extrabold text-white">{(features.mean_response_time_ms / 1000).toFixed(1)}s</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Variance:</span>
                      <span className="font-extrabold text-white">{features.response_time_variance}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Repeat Errors:</span>
                      <span className="font-extrabold text-white">{features.repeat_error_rate}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Corrections:</span>
                      <span className="font-extrabold text-white">{features.correction_rate}</span>
                    </div>
                  </div>
                ) : (
              ...
              <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800">
                <span className="text-[10px] font-black uppercase text-blue-400 tracking-wider block mb-2">
                ...
                {inferenceResult ? (
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-extrabold text-emerald-400">⚡ {inferenceResult.inference_latency_ms}ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Decision:</span>
                      <span className={`font-black ${
                        inferenceResult.recommendation === 'DECREASE' ? 'text-amber-400' : 'text-emerald-400'
                      }`}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-extrabold text-white">{Math.round(inferenceResult.confidence * 100)}%</span>
                    </div>
                  ...
                  <span className="text-xs text-slate-500">Awaiting inference...</span>
                )}
              </div>
                ...
                {baselineMetrics ? (
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className={`font-black ${
                        baselineMetrics.status === 'MEANINGFUL_DEVIATION' ? 'text-amber-400' : 'text-emerald-400'
                      }`}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-black text-blue-400">Level {officeKitPacket?.adaptation.recommendedDifficulty}</span>
                    </div>
                  ...
                  <span className="text-xs text-slate-500">Awaiting evaluation...</span>
                )}
              </div>
            </div>
            ...
            {officeKitPacket && (
              <div className="mt-6 p-4 rounded-2xl bg-indigo-950/60 border border-indigo-500/40 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div>
        ...
        <div className="p-6 rounded-3xl bg-slate-950 border border-slate-800 text-xs text-slate-400 leading-relaxed">
